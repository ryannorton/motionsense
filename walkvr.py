...
def main():
	...
	while True:
                ...
                # get serial input
                bytes = ser.read(2)
                step = int(bytes) #casts character bytes to integer

		# check for jump
		if ((step == 12 and true_prev_step == 22) or (step == 22 and true_prev_step == 12)) and (curr_time - prev_time < .2): # both feet went up within .2 seconds of eachother
			print "jump"
			true_prev_step = step
